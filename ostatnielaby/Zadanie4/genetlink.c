...
{
	if(info && info->attrs)
		sprintf(content,"Received message: \"%s\", sequence number: %u port ID: %d\n",(char *)nla_data(info->attrs[TEST_GENETLINK_A_MSG]),info->snd_seq,info->snd_portid);
	return 0;
}
...
{
	test_genetlink_policy[TEST_GENETLINK_A_MSG].type = NLA_STRING;
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("A user space to kernel space simple genetlink communication module.");
MODULE_VERSION("1.0");
