...
static int deviceSize = 4*1024*1024;
module_param(deviceSize, int, 0644);
MODULE_PARM_DESC(deviceSize, "RAM-disk_size");
...
{
	sbd_dev.memory = vmalloc(deviceSize);
	if(!sbd_dev.memory) {
	...
	set_capacity(sbd_dev.gd,(deviceSize)>>9);
